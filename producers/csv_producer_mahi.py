...
def get_message_interval() -> int:
    ...
    interval = int(os.getenv("FOOD_INTERVAL_SECONDS", 1))
...
DATA_FILE = DATA_FOLDER.joinpath("food.csv")
        ...
        try:
            ...
            with open(DATA_FILE, "r") as csv_file:
                ...
                for row in csv_reader:
                    ...
                    if "food_item" not in row or "quantity" not in row:
                        logger.error(f"Missing 'food_item' or 'quantity' column in row: {row}")
                        continue
